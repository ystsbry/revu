...
スキルはすべて `plugin/` 配下の revu プラグインとして配布します。レビュー生成の `revu:pr`・レビューデータ編集の `revu:edit`・レビュー閲覧の `revu:read`・レビュー削除の `revu:delete`・傾向レポートの `revu:report`・プロファイル作成の `revu:create-profile` の 6 スキル構成で、同じプラグインを Claude Code と OpenAI Codex CLI の両方から呼べます。skill 本体は 1 つで、ランタイムだけ差し替わります。

...
各指摘の severity は **Step 6 で取得した定義集合**から選ぶ。`description` を読んで対応する `name` を割り当て、`level` の数値感も判断材料にする。
...
`{severity}` は Step 6 で解決した severity セットの `name` のいずれか。`review.yml` の `comments[].severity` と必ず一致させること。GitHub に投稿された後、コメント本文だけ読んでも重大度が分かるようにするための表記。
...
comments:
  - id: c1
    status: pending
    severity: {Step 6 の severity name のいずれか}
...
- 各コメントの severity に紐づく `review_event`（Step 6 の JSON で取得済）を集める

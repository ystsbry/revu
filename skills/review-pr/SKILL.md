---
name: review-pr
description: GitHub PR を読解し、revu が消費できる形式 (~/.revu/{owner}/{repo}/pr-{N}/{sha[:7]}/) に review.yml + summary.md + comments/*.md を生成する。「PR をレビューして」「review-pr」「PR #123 のレビューを下書きして」などと言われたら使う。引数は <PR_NUMBER> と任意の --focus <categories>。
---

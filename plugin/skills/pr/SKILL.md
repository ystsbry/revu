---
name: pr
description: GitHub PR を読解し、revu が消費できる形式 (~/.revu/{owner}/{repo}/pr-{N}/{sha[:7]}/) に review.yml + summary.md + comments/*.md を生成する。「PR をレビューして」「revu:pr」「PR #123 のレビューを下書きして」などと言われたら使う。引数は <PR_NUMBER> と任意の --focus <categories>。
---
...
```
/revu:pr <PR_NUMBER>
/revu:pr <PR_NUMBER> --focus <categories>
```
...
以下のディレクトリ構造を `~/.revu/{owner}/{repo}/pr-{N}/{sha[:7]}/` 配下に生成:
...
`{owner}/{repo}` は PR のベースリポジトリ、`{sha[:7]}` は PR の `head_sha` の先頭 7 文字、`{filename}` は対象ファイルのベース名（拡張子なし、英数字に正規化）。
SHA を dir に含めることで、PR の force-push などで head_sha が動いても過去のレビュー dir を上書きしない。
...
SUMMARY_TMPL=$(revu templates path summary.md.tmpl 2>/dev/null) \
  || SUMMARY_TMPL="$HOME/.claude/plugins/revu/skills/pr/templates/summary.md.tmpl"

INLINE_TMPL=$(revu templates path inline-comment.md.tmpl 2>/dev/null) \
  || INLINE_TMPL="$HOME/.claude/plugins/revu/skills/pr/templates/inline-comment.md.tmpl"
```
...
- `review_event` は Step 11 の review_event 判定に使う
...
- 一般観点（次節の `bug` / `design` / ... の表）と矛盾しない場合は両方を満たす
- ガイドラインに明示的に書かれているルールは、それを根拠としてコメントで参照してよい（例: 「`docs/style.md` の "命名" 節に従い ...」）
- ガイドラインの主旨が一般観点と衝突する場合は **ガイドライン側を優先**（プロジェクト固有のルールがある以上、それが意思決定者）

`revu` が `$PATH` に無い、または `revu guidelines paths` が空のときは、次節の一般観点だけで進める。
...
- `comments[].start_line`: **範囲指定するときに必須**。Step 7 のガイドラインに従い、指摘が複数行に跨るなら省略せず付ける。同一 side のときは `start_side` を省略してよい（自動的に `side` と同じ扱い）。**削除行 (`-`) と追加行 (`+`) を跨いで範囲指定したいとき** は `start_side: LEFT` + `side: RIGHT` のように両方を明示する（同一 side では `start_line <= line` 制約あり、跨ぐ場合は制約なし）
...
```
Generated review at ~/.revu/{owner}/{repo}/pr-<N>/<sha[:7]>/
- summary.md
- {N} inline comments
- review_event: {APPROVE | COMMENT | REQUEST_CHANGES}
...
- **書き換えてはいけないもの**: 同一 SHA で再生成すると `~/.revu/{owner}/{repo}/pr-<N>/{sha[:7]}/` 配下を上書きする。`submitted_at` が記録されている場合は、そのまま上書きせずユーザーに確認を取る。head_sha が変わっていれば自然に別 dir に書かれるので衝突しない
